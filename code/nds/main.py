...
from .back_diff import KthOrderBackDiff
...
def main(args):
    ...
    with torch.no_grad():
        ...
        try:
            ...
            kbd = KthOrderBackDiff(H=H, W=W, C=C, K=K)
            ...
            cmap = None
            ims = [axs[k].imshow(activation(kbd.state[k][-1]), cmap=cmap) for k in range(K + 1)]

            ## set up animation
            def draw_func(frame):
                t, can = frame
                for k, im in enumerate(ims):
                    im.set_data(activation(kbd.state[k][-1]))
